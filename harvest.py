...
def make_external_url(code_url: str, release: str, path: str) -> str:
    ...
    if "github.com" in code_url:
        return f"{code_url}/blob/{release}/{path}"
...
def config_from_code(item: CatalogItem) -> Config:
    ...
    if 'logo' in config:
        ...
        rel_logo = config['sphinx']['config']['html_theme_options']['logo']['image_light']
        static_path = config['sphinx']['config']['html_static_path'][0]
        logo = make_download_url(item.code_url, item.release, f"{static_path}/{rel_logo}")
...
def content_entry(
    ...
    if "file" in entry:
        ...
        return TocEntry(
            ...
            external_url=make_external_url(item.code_url, item.release, entry["file"]),
            children=[],
        )
...
def merge_tocs(item: CatalogItem, toc_yml, toc_html: list[tuple[str, str]]):
    ...
    root_code_url = make_external_url(item.code_url, item.release, root_path)
    toc = TocEntry(title='', html_url=item.html_url, external_url=root_code_url, children=[])
    if "parts" in toc_yml:
                            ...
                            for subsection in section["sections"]:
                                ...
                                if 'section' in subsection:
                                    for subsubsection in subsection['sections']:
                                        ...
                                        subsection_toc.children.append(subsubsection_toc)
...
def main():
    parser = argparse.ArgumentParser(
        prog="harvest",
        description="Harvest toc catalog items"
    )
    parser.add_argument('--catalog', type=Path, help='Path to catalog file', default="catalog.yml")
    parser.add_argument('--output', '-o', type=Path, help='Path to output file', default="catalog.json")
    ...
    with open(args.catalog) as f, open(args.output, "w") as f_out:
        ...
        for catalog_item in catalog:
            ...
            items.append(item)
            logger.info(f"Processed {catalog_item['html_url']}")
